...
let planet = null;
let tileSelection = null;
let zoom = 1.0;
let zoomAnimationStartTime = null;
let zoomAnimationDuration = null;
let zoomAnimationStartValue = null;
let zoomAnimationEndValue = null;
let cameraLatitude = 0;
let cameraLongitude = 0;
let surfaceRenderMode = 'terrain';
let renderSunlight = true;
let renderPlateBoundaries = false;
let renderPlateMovements = false;
let renderAirCurrents = false;
let sunTimeOffset = 0;
const pressedKeys = {};
...
function setPlateCount(plateCount) {
    if (typeof (plateCount) === 'number' && plateCount >= 0) {
        ...
        const sliderVal = Math.ceil((Math.log(plateCount) / Math.log(2) - 1) / (Math.log(1000) / Math.log(2) - 1) * 300).toFixed(0);
...
function generatePlanetAsynchronous() {
    ...
    const subdivisions = generationSettings.subdivisions;
...
function generatePlanetMesh(icosahedronSubdivision, topologyDistortionRate, random, action) {
    ...
    action.executeSubaction(function (action) {
        ...
        let priorShift;
        let currentShift = relaxMesh(mesh, 0.5, action);
        action.executeSubaction(function (action) {
...
function generateSubdividedIcosahedron(degree) {
    ...
    for (var i = 0; i < icosahedron.faces.length; ++i) {
        ...
        const getEdgeNode0 = (face.n[0] === edge0.n[0]) ?
            function (k) {
...
function generatePlanetElevation(topology, plates, action) {
    let boundaryCorners;
    let boundaryCornerInnerBorderIndexes;
    let elevationBorderQueue;
    const elevationBorderQueueSorter = function (left, right) {
...
function calculatePlateBoundaryStress(boundaryCorners, action) {
    ...
    for (let i = 0; i < boundaryCorners.length; ++i) {
        ...
        if (innerBorder) {
            ...
            const stress = calculateStress(plate0.calculateMovement(corner.position), plate1.calculateMovement(corner.position), boundaryVector, boundaryNormal);
            ...
            const stress0 = calculateStress(plate0.calculateMovement(corner.position), plate1.calculateMovement(corner.position), boundaryVector0, boundaryNormal0);
            const stress1 = calculateStress(plate1.calculateMovement(corner.position), plate2.calculateMovement(corner.position), boundaryVector1, boundaryNormal1);
            const stress2 = calculateStress(plate2.calculateMovement(corner.position), plate0.calculateMovement(corner.position), boundaryVector2, boundaryNormal2);
...
function calculateDormantElevation(distanceToPlateBoundary, distanceToPlateRoot, boundaryElevation, plateElevation, pressure, shear) {
    ...
    const a = 2 * elevationDifference;
    const b = -3 * elevationDifference;
...
function generatePlanetWeather(topology, partitions, heatLevel, moistureLevel, random, action) {
    ...
    action
        ...
        .executeSubaction(function (action) {
            const consumedHeat = processAirHeat(activeCorners, action);
        ...
        .executeSubaction(function (action) {
            const consumedMoisture = processAirMoisture(activeCorners, action);
...
function buildSurfaceRenderObject(tiles, random, action) {
    const planetGeometry = new THREE.Geometry();
    const terrainColors = [];
    const plateColors = [];
    ...
    action.executeSubaction(function (action) {
        ...
        const moistureColor = new THREE.Color(0xFFCC00).lerp(new THREE.Color(0x0066FF), Math.max(0, Math.min(tile.moisture, 1)));
        ...
        for (let j = 0; j < tile.corners.length; ++j) {
            ...
            for (let k = planetGeometry.faces.length - 3; k < planetGeometry.faces.length; ++k) planetGeometry.faces[k].vertexColors = terrainColors[k];
        }
...
function buildPlateBoundariesRenderObject(borders, action) {
    ...
    action.executeSubaction(function (action) {
        ...
        if (border.betweenPlates) {
            ...
            const innerColor = (pressure <= 0) ? new THREE.Color(1 + pressure, 1, 0) : new THREE.Color(1, 1 - pressure, 0);
            const outerColor = new THREE.Color(0, shear / 2, shear);
    ...
    const renderObject = new THREE.Mesh(geometry, material);
...
function buildPlateMovementsRenderObject(tiles, action) {
    ...
    action.executeSubaction(function (action) {
        ...
        const plateMovementColor = new THREE.Color(1 - plate.color.r, 1 - plate.color.g, 1 - plate.color.b);
...
function generatePlanetStatistics(topology, plates, action) {
    ...
    const updateMinMaxAvg = function (stats, value) {
...
SteppedAction.prototype.step = function SteppedAction_step() {
    ...
    while (Date.now() < this.intervalEndTime && !this.canceled && !this.completed) {
        const action = this.callStack.actions[this.callStack.index];
...
let lastRenderFrameTime = null;

function getZoomDelta() {
    const zoomIn = (pressedKeys[KEY_NUMPAD_PLUS] || pressedKeys[KEY_PAGEUP]);
    const zoomOut = (pressedKeys[KEY_NUMPAD_MINUS] || pressedKeys[KEY_PAGEDOWN]);
...
function getLatitudeDelta() {
    const up = (pressedKeys[KEY.W] || pressedKeys[KEY.Z] || pressedKeys[KEY_UPARROW]);
    const down = (pressedKeys[KEY.S] || pressedKeys[KEY_DOWNARROW]);
    if (up && !down) return +1;
    if (down && !up) return -1;
    return 0;
}
...
function getLongitudeDelta() {
    const left = (pressedKeys[KEY.A] || pressedKeys[KEY.Q] || pressedKeys[KEY_LEFTARROW]);
    const right = (pressedKeys[KEY.D] || pressedKeys[KEY_RIGHTARROW]);
...
function render() {
    ...
    const frameDuration = lastRenderFrameTime !== null ? (currentRenderFrameTime - lastRenderFrameTime) * 0.001 : 0;
    ...
    const cameraLongitudeDelta = getLongitudeDelta();
    ...
    const sunTime = Math.PI * 2 * currentRenderFrameTime / 60000 + sunTimeOffset;
...
function updateCamera() {
    ...
    const transformation = new THREE.Matrix4().makeRotationFromEuler(new THREE.Euler(cameraLatitude, cameraLongitude, 0, 'YXZ'));
...
function clickHandler(event) {
    if (planet) {
        const x = event.pageX / renderer.domElement.width * 2 - 1;
        const y = 1 - event.pageY / renderer.domElement.height * 2;
        const rayCaster = projector.pickingRay(new Vector3(x, y, 0), camera);
        const intersection = planet.partition.intersectRay(rayCaster.ray);
...
function setSurfaceRenderMode(mode, force) {
    if (mode !== surfaceRenderMode || force === true) {
        ...
        const faces = planet.renderData.surface.geometry.faces;
...
function showHideSunlight(show) {
    ...
    const material = planet.renderData.surface.material;
    if (renderSunlight) {
